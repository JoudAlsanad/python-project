...
class Forms:
    ...
    def saveUserInf(self):
        try:
            ...
            if len(id.fetchall()) == 0:
                c.execute(f"insert into userinf values('{userid}','{first_name}','{last_name}','{password}','{email}','{mobile}')")
                connection.commit()
    ...
    def MatchUserInf(self):
        try:
            ...
            # add user information to the database
            id = c.execute(f"SELECT user_id FROM userinf WHERE user_id = {userid} and password={self.password_entry.get()}")
            if len(id.fetchall()) == 0:
    ...
    def saveActivitiesInf(self):
        try:
            ...
            if len(id.fetchall()) == 0:
                c.execute(f"insert into eventsinf values('{event_id}','{event_name}','{event_location}','{capacity}','{event_date}','{event_time}')")
                connection.commit()
    ...
    def CreateBookingForm(self):
        ...
        # Book tickit details
        Label(Book_Tickit_Tab, text= "Active Events", font= ('Helvetica 20 bold')).grid(column=0,row=0, columnspan = 4,padx=10,pady=10)
        ...
        self.book_button = ttk.Button(Book_Tickit_Tab, text='Create', command=self.saveBookingInf).grid(column=0,row=2,padx=10,pady=10, sticky = E, columnspan = 2)
        self.logout_button2 = ttk.Button(Book_Tickit_Tab, text='Logout', command=self.logout).grid(column=2,row=2,sticky = W, columnspan = 2)

        # second Tree view 
        Label(My_tickits_Tab, text= "My Tickits", font= ('Helvetica 20 bold')).grid(column=0,row=0, columnspan = 4,padx=10,pady=10)
        ...
        self.fillTreeviewActiveTic()
        self.submit_button = ttk.Button(My_tickits_Tab, text='Show', command=self.fillTreeviewMyTic).grid(column=0,row=2,padx=10,pady=10, sticky = E, columnspan = 2)
        self.login_button = ttk.Button(My_tickits_Tab, text='Logout', command=self.logout).grid(column=2,row=2,sticky = W, columnspan = 2)
    ...
    def fillTreeviewActiveTic(self):
        connection = sqlite3.connect('TSDatabase.db')
        cursor = connection.execute(f"SELECT event_id,event_Name,event_location,capacity,event_date,event_time from eventsinf where event_date > {date.today()}")
        count = 0
        ...
        for row in cursor:
            ...
            if b>=datetime.now():
                self.atcivites_treeView.insert(parent='', index=count, text='', values=(row[0], row[1], row[2],row[3],row[4],row[5]))
                count+=1
            else:
                count+=1
        return
    def fillTreeviewMyTic(self):
        connection = sqlite3.connect('TSDatabase.db')
        cursor = connection.execute(f"select  first_Name,tickit.tickit_id,event_Name ,event_location,event_date,event_time  from userinf,eventsinf,tickit where userinf.user_id=tickit.user_id and eventsinf.event_id=tickit.event_id and tickit.user_id= { self.generalvar }")
        count = 0
        for i in self.myTickit_treeView.get_children():
                self.myTickit_treeView.delete(i)
        for row in cursor:
            self.myTickit_treeView.insert(parent='', index=count, text='', values=(row[0], row[1], row[2],row[3],row[4],row[5]))
            count+=1
        connection.close()
        return
